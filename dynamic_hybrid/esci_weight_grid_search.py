#!/usr/bin/env python3
"""
ESCI Weight Grid Search Experiment

E-commerce Product Search dataset validation for LLM Ground Truth experiment.
ESCI domain: Amazon shopping queries
...
Usage:
    python dynamic_hybrid/esci_weight_grid_search.py \
        --host <opensearch_host> \
        --port 80 \
        --embedding-model-id <model_id> \
        --num-queries 100 \
        --cache-file esci_llm_cache_100q.json
"""
...
# ============================================================================
# DATA LOADING (ESCI-specific formats)
# ============================================================================
...
def load_esci_queries(queries_path: str) -> Dict[str, ESCIQuery]:
    """Load ESCI queries from JSON file."""
    queries = {}
...
def load_esci_qrels(qrels_path: str) -> Dict[str, Dict[str, int]]:
    """Load ESCI relevance judgments (qrels) from TSV file (TREC 4-column format)."""
...
# ============================================================================
# LLM JUDGE (with cache from 3way experiment)
# ============================================================================
...
class LLMJudge:
    ...
    def rate_relevance(self, query: str, product_title: str, product_desc: str, query_id: str, doc_id: str) -> float:
        """Rate relevance of product to shopping query on 0-1 scale."""
        ...
        prompt = f"""Rate how relevant this product is to the given shopping query.

SHOPPING QUERY: {query}

PRODUCT TITLE: {product_title}
PRODUCT DESCRIPTION: {product_desc[:1500]}
...
# ============================================================================
# HYBRID SEARCH (ESCI-specific fields)
# ============================================================================

class HybridSearcher:
    """Execute hybrid searches with configurable weights for ESCI index."""
    ...
    def __init__(
        ...
        neural_field: str = "info_embedding",  # ESCI uses info_embedding
    ...
    def fetch_documents(self, doc_ids: List[str]) -> Dict[str, Dict]:
        """Fetch document content for LLM evaluation."""
        if not doc_ids:
            return {}
        ...
        query_body = {
            ...
            "size": len(doc_ids),
            "_source": ["product_title", "product_description", "product_brand"]
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for hit in results.get('hits', {}).get('hits', []):
                    docs[hit['_id']] = hit.get('_source', {})
                return docs
            return {}
        except Exception as e:
            print(f"  [ERROR] Fetch documents exception: {str(e)}")
            return {}
...
class GridSearchEngine:
    """Run weight grid search experiment for ESCI dataset."""
    
    WEIGHTS = [round(w * 0.1, 1) for w in range(11)]  # [0.0, 0.1, ..., 1.0]
    LLM_RELEVANCE_THRESHOLD = 0.6  # Products with LLM score >= 0.6 are "relevant"
    ...
    def run(self, query_ids: List[str]) -> Tuple[List[WeightMetrics], List[WeightMetrics]]:
        ...
        for i, qid in enumerate(query_ids):
            ...
            uncached_docs = [d for d in pooled_doc_ids if d not in self.llm_ratings_cache[qid]]
            
            if uncached_docs:
                # Fetch document content from OpenSearch
                docs = self.searcher.fetch_documents(uncached_docs)
                ...
                for doc_id in uncached_docs:
                    if doc_id in docs:
                        ...
                        # Combine brand with title for better context
                        full_title = f"{brand} - {title}" if brand else title
                        
                        rating = self.llm_judge.rate_relevance(
                            query.text, full_title, desc, qid, doc_id
                        )
                        self.llm_ratings_cache[qid][doc_id] = rating
                    else:
                        # Document not found in index, use default
                        self.llm_ratings_cache[qid][doc_id] = 0.0
        ...
        for neural_weight in self.WEIGHTS:
            ...
            for qid in query_ids:
                ...
                # Human ground truth metrics (ESCI uses 0-3 scale: E=3, S=2, C=1, I=0)
                ...
                if human_rel:
                    ...
                    # For recall, consider E=3 and S=2 as "relevant" (score >= 2)
                    human_recall_1.append(compute_recall(ranking, human_rel, k=1, relevance_threshold=2))
                    human_recall_10.append(compute_recall(ranking, human_rel, k=10, relevance_threshold=2))
                    human_recall_25.append(compute_recall(ranking, human_rel, k=25, relevance_threshold=2))
                
                # LLM ground truth metrics
                llm_rel = self.llm_ratings_cache.get(qid, {})
                if llm_rel:
                    # Scale LLM ratings (0-1) to match human grade scale (0-3)
                    llm_rel_scaled = {doc_id: score * 3 for doc_id, score in llm_rel.items()}
...
def print_results(human_metrics: List[WeightMetrics], llm_metrics: List[WeightMetrics], num_queries: int):
    ...
    print("\n" + "="*100)
    print("ESCI WEIGHT GRID SEARCH RESULTS")
    print(f"Configuration: min_max + arithmetic_mean, {num_queries} queries")
    print("="*100)
    
    # Table A: Human Ground Truth
    print("\n[A] HUMAN GROUND TRUTH (Amazon ESCI labels: E=3, S=2, C=1, I=0)")
    print("-"*100)
...
def save_results(
    ...
    results = {
        ...
        "configuration": {
            "dataset": "ESCI",
            "domain": "E-commerce product search (Amazon)",
            "normalization": "min_max",
            "combination": "arithmetic_mean",
            "llm_model": "gpt-3.5-turbo (via ML Commons or OpenAI)",
            "llm_relevance_threshold": 0.6,
            "num_weights": 11,
            "cutoffs": [1, 10, 25],
            "human_relevance_labels": "E=3, S=2, C=1, I=0"
        }
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="ESCI Weight Grid Search Experiment"
    )
    ...
    parser.add_argument("--cache-file", default="esci_llm_cache_100q.json", help="LLM cache file")
    parser.add_argument("--num-queries", type=int, default=100, help="Number of queries")
    parser.add_argument("--output", default="esci_weight_grid_results.json", help="Output file")
    parser.add_argument("--debug", action="store_true", help="Enable debug output")
    
    args = parser.parse_args()
    
    print("="*80)
    print("ESCI WEIGHT GRID SEARCH EXPERIMENT")
    print("E-commerce Product Search (Amazon) - LLM Ground Truth Validation")
    print("="*80)
    ...
    searcher = HybridSearcher(
        ...
        neural_field="info_embedding",  # ESCI uses info_embedding
        # Fields from hybrid_field_detector.py: use only 2 text fields with title boost
        # Excludes product_brand/product_bullet_points to avoid double-counting
