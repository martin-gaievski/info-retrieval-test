#!/usr/bin/env python3
"""
TREC-COVID 3-Way Comparison Experiment with LLM Ground Truth Analysis

Compares three ranking approaches and evaluates synthetic ground truth:
1. Hybrid Search (per config) - direct OpenSearch hybrid query
2. LLM-Pooled - pooled docs from multiple configs, ranked by LLM
3. Human Labels - ground truth from TREC-COVID qrels

NEW: Also computes optimal weights using LLM ratings as ground truth
and compares to optimal weights from human ground truth.

Usage:
    # First run - collects LLM ratings and saves to cache
    python dynamic_hybrid/trec_covid_3way_comparison.py \
...
# Scientific/Medical domain prompt for TREC-COVID
SCIENTIFIC_SYSTEM_PROMPT = """You are an expert biomedical/scientific literature relevance evaluator. Your task is to rate how relevant each scientific paper is to a given research question about COVID-19.

Rating Scale (0.0 to 1.0 with 0.2 increments):
- 1.0: Perfect Match - Paper directly and thoroughly addresses the research question
- 0.8: Excellent - Highly relevant paper, provides substantial information for the research question
- 0.6: Good - Relevant paper, partially addresses the research question
- 0.4: Fair - Some relevance but only tangentially related to the research question
- 0.2: Poor - Marginally related paper, mostly off-topic
- 0.0: Not Relevant - Completely unrelated to the research question

Instructions:
1. Evaluate each paper independently based on title and abstract
2. Consider semantic relevance to the COVID-19 research question
3. Papers about related coronaviruses (SARS, MERS) may be relevant if the question is about COVID-19
...
class ComparisonResult:
    ...
    # NEW: LLM-as-ground-truth metrics
...
def compute_ndcg(ranking: List[str], relevance: Dict[str, float], k: int = 10) -> float:
    ...
    Args:
        ...
        relevance: Dict of doc_id -> relevance score (can be int or float)
        k: Cutoff position
    ...
    for i, doc_id in enumerate(ranking[:k]):
        ...
        dcg += (2**rel - 1) / np.log2(i + 2)  # i+2 because positions are 1-indexed
...
class TRECCovidComparison:
    """3-way comparison engine for TREC-COVID dataset."""
    ...
    def __init__(
        ...
        # Field mapping for TREC-COVID index
        # NOTE: passage_text is a concatenation of title_key + text_key, so we use
        # atomic fields only to avoid BM25 double-counting
        ...
        self.llm_judge = LLMJudge(
            ...
            temperature=0.0,
            batch_size=15,  # Smaller batches for longer scientific text
            debug=debug
        )
    ...
    def run_comparison(
        ...
        Args:
            query: The TREC-COVID query
            human_relevance: Human relevance judgments for this query
            pool_depth: Number of docs to retrieve per config
            use_cache_only: If True, fail if not in cache (don't call LLM)
        ...
        if cached:
            ...
            for doc_id, doc in pooled_docs.items():
                docs_for_llm[doc_id] = {
                    ...
                    "text": doc.get(self.text_field, "")[:800]  # Truncate long abstracts
                }
        ...
        # NEW: Step 6: Compute NDCG for each config vs LLM ratings as ground truth
        # Convert LLM scores to be used as ground truth (scale 0-1 to 0-2 for comparable DCG)
...
def print_summary(results: List[ComparisonResult], configs: List[str]):
    ...
    # ============================================================
    # NEW SECTION: LLM as Ground Truth Analysis
    # ============================================================
    print("\n" + "="*80)
    ...
    # Confusion matrix of optimal configs
    print("\n[8] Confusion Matrix: Best Config (Human GT) → Best Config (LLM GT):")
    print("-" * 60)
    confusion = defaultdict(lambda: defaultdict(int))
    for result in results:
        confusion[result.best_hybrid_config][result.best_config_llm_gt] += 1
    
    # Print header
    all_configs = sorted(set(list(best_counts_human.keys()) + list(best_counts_llm.keys())))
    print(f"    {'Human GT →':<18}", end="")
    for config in all_configs:
        print(f"{config[:8]:>10}", end="")
    print()
    
    for human_config in all_configs:
        print(f"    {human_config:<18}", end="")
        for llm_config in all_configs:
            count = confusion[human_config][llm_config]
...
def main():
    parser = argparse.ArgumentParser(
        description="TREC-COVID 3-Way Comparison Experiment with LLM Ground Truth Analysis"
    )
    ...
    print("="*80)
    print("TREC-COVID 3-WAY COMPARISON EXPERIMENT")
    print("With LLM Ground Truth Analysis")
    print("="*80)
