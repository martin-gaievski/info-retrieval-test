#!/usr/bin/env python3
"""
Extract CQADupStack Android queries with ultra-aggressive sanitization for OpenSearch.
"""
...
def create_opensearch_import_format(queries: Dict[str, str]) -> Dict:
    ...
    opensearch_import = {
        "name": "cqadupstack_android_queries",
        "description": f"CQADupStack Android containing {len(query_set_queries)} Stack Exchange Android development queries from the BEIR benchmark dataset",
        "sampling": "manual",
...
def main():
    """Main function to extract and format CQADupStack Android queries."""
    
    # File paths
    queries_file = "datasets/cqadupstack/android/queries.jsonl"
    output_file = "results/cqadupstack_android_queries.json"
    
    print("=" * 80)
    print("CQADupStack Android Query Extraction with ULTRA-AGGRESSIVE Sanitization")
    print("=" * 80)
    
    # Check if file exists
    if not os.path.exists(queries_file):
        print(f"\n❌ Error: File not found: {queries_file}")
        print("   Please ensure the CQADupStack dataset is properly extracted.")
        return
    
    # Step 1: Load all queries
    print("\n1. Loading Android queries...")
