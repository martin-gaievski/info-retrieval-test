#!/usr/bin/env python3
"""
Extract DBPedia queries with ultra-aggressive sanitization for OpenSearch.
"""
...
def create_opensearch_import_format(queries: Dict[str, str]) -> Dict:
    ...
    opensearch_import = {
        "name": "dbpedia_queries",
        "description": f"DBPedia Entity containing {len(query_set_queries)} queries from the BEIR benchmark dataset",
        "sampling": "manual",
...
def main():
    """Main function to extract and format DBPedia queries."""
    
    # File paths
    queries_file = "datasets/dbpedia-entity/queries.jsonl"
    output_file = "results/dbpedia_queries.json"
    
    print("=" * 80)
    print("DBPedia Entity Query Extraction with ULTRA-AGGRESSIVE Sanitization")
    print("=" * 80)
