#!/usr/bin/env python3
"""
Extract CLIMATE-FEVER queries with ultra-aggressive sanitization for OpenSearch.
...
def create_opensearch_import_format(queries: Dict[str, str], qrels_query_ids: Set[str]) -> Dict:
    ...
    opensearch_import = {
        "name": "climate_fever_queries",
        "description": f"CLIMATE-FEVER containing {len(query_set_queries)} climate-related fact-checking queries from the BEIR benchmark dataset",
        "sampling": "manual",
...
def validate_output(opensearch_data: Dict) -> None:
    ...
    print("\n6. Validating output is pure ASCII...")
...
def main():
    """Main function to extract and format CLIMATE-FEVER queries."""
    
    # File paths
    queries_file = "datasets/climate-fever/queries.jsonl"
    qrels_file = "datasets/climate-fever/qrels/test.tsv"
    output_file = "results/climate_fever_queries.json"
    
    print("=" * 80)
    print("CLIMATE-FEVER Query Extraction with ULTRA-AGGRESSIVE Sanitization")
    print("=" * 80)
    
    # Check if file exists
    if not os.path.exists(queries_file):
        print(f"\n❌ Error: File not found: {queries_file}")
        print("   Please ensure the CLIMATE-FEVER dataset is properly extracted.")
        return
    ...
    print("\n2. Loading CLIMATE-FEVER queries...")
