#!/usr/bin/env python3
"""
Extract DBPedia relevance judgments with ultra-aggressive sanitization for OpenSearch.
Maps 3-point scale (0, 1, 2) to (0.0, 0.33, 1.0)
"""
...
def sanitize_doc_id(doc_id: str) -> str:
    """Keep document IDs in their original format as expected by OpenSearch."""
    # Keep the original format: <dbpedia:EntityName>
    # Only strip whitespace, preserve angle brackets and dbpedia: prefix
    return doc_id.strip()
...
def load_qrels(qrels_file: str) -> Dict[str, List[Dict[str, str]]]:
    ...
    Maps DBPedia's 3-point scale to OpenSearch's scale:
    - 0 → 0.0 (not relevant)
    - 1 → 0.33 (somewhat relevant)
    - 2 → 1.0 (highly relevant)
    
    Returns a dictionary mapping query_id to a list of {docId, rating} dicts.
    """
    qrels = defaultdict(list)
    
    # Rating mapping from DBPedia scale to OpenSearch scale
    rating_map = {
        0: 0.0,    # Not relevant
        1: 0.33,   # Somewhat relevant  
        2: 1.0     # Highly relevant
    ...
    with open(qrels_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            if len(row) >= 3:
                query_id = row[0]
                doc_id = sanitize_doc_id(row[1])  # Clean up document ID
                score = int(row[2])
...
def create_opensearch_judgments_format(qrels: Dict, all_queries: Dict) -> Dict:
    ...
    opensearch_judgments = {
        "name": "DBPedia Entity Relevance Judgments",
        "description": f"DBPedia Entity containing relevance judgments for {len(judgment_ratings)} queries from the BEIR benchmark dataset",
...
def main():
    """Main function to extract and format DBPedia judgments."""
    
    # File paths
    queries_file = "datasets/dbpedia-entity/queries.jsonl"
    qrels_file = "datasets/dbpedia-entity/qrels/test.tsv"
    output_file = "results/dbpedia_judgments.json"
    
    print("=" * 80)
    print("DBPedia Entity Judgment Extraction with ULTRA-AGGRESSIVE Sanitization")
    print("=" * 80)
    ...
    print("   Mapping ratings: 0 → 0.0, 1 → 0.33, 2 → 1.0")
    ...
    print(f"   Note: DBPedia uses 3-point scale (0=not relevant, 1=somewhat relevant, 2=highly relevant)")
    print(f"   Mapped to OpenSearch scale: 0.0, 0.33, 1.0")
    ...
    if all_scores:
        ...
        for score in sorted(score_dist.keys()):
            ...
            if score == '0.0':
                ...
                print(f"    Score 0.33 (somewhat relevant): {count} documents")
            elif score == '1.0':
                print(f"    Score 1.0 (highly relevant): {count} documents")
            else:
