#!/usr/bin/env python3
"""
Extract CQADupStack relevance judgments with ultra-aggressive sanitization for OpenSearch.
Maps binary scale (0, 1) to (0.0, 1.0)
Supports all 12 subsets: android, english, gaming, gis, mathematica, physics, 
...
def load_qrels(qrels_file: str) -> Dict[str, List[Dict[str, str]]]:
    ...
    Maps CQADupStack's binary scale to OpenSearch's scale:
    - 0 → 0.0 (not relevant)
    - 1 → 1.0 (relevant)
    ...
    # Rating mapping from CQADupStack binary scale to OpenSearch scale
    rating_map = {
        0: 0.0,    # Not relevant
        1: 1.0     # Relevant
    }
...
def create_opensearch_judgments_format(qrels: Dict, all_queries: Dict, subset: str) -> Dict:
    ...
    subset_display = subset.title()
    opensearch_judgments = {
        "name": f"CQADupStack {subset_display} Relevance Judgments",
        "description": f"CQADupStack {subset_display} containing relevance judgments for {len(judgment_ratings)} Stack Exchange queries from the BEIR benchmark dataset",
...
def main():
    """Main function to extract and format CQADupStack judgments."""
    
    # Parse command-line arguments
    parser = argparse.ArgumentParser(
        description='Extract CQADupStack judgments for OpenSearch import',
...
Examples:
  python3 {sys.argv[0]} android
  python3 {sys.argv[0]} --subset english
  python3 {sys.argv[0]} -s gaming
        """
    )
    parser.add_argument(
        'subset',
        ...
        help='CQADupStack subset to process'
    )
    parser.add_argument(
        '-s', '--subset',
        dest='subset_flag',
        choices=AVAILABLE_SUBSETS,
        help='CQADupStack subset to process (alternative syntax)'
    )
    
    args = parser.parse_args()
    
    # Determine which subset to use
    subset = args.subset or args.subset_flag
    if not subset:
        parser.print_help()
        print(f"\n❌ Error: Please specify a subset to process")
        print(f"   Available subsets: {', '.join(AVAILABLE_SUBSETS)}")
        sys.exit(1)
    
    # File paths
    queries_file = f"datasets/cqadupstack/{subset}/queries.jsonl"
    qrels_file = f"datasets/cqadupstack/{subset}/qrels/test.tsv"
    output_file = f"results/cqadupstack_{subset}_judgments.json"
    
    print("=" * 80)
    print(f"CQADupStack {subset.title()} Judgment Extraction with ULTRA-AGGRESSIVE Sanitization")
    print("=" * 80)
    
    # Check if files exist
    if not os.path.exists(queries_file):
        print(f"\n❌ Error: Queries file not found: {queries_file}")
        print(f"   Please ensure the CQADupStack {subset} dataset is properly extracted.")
        sys.exit(1)
    
    if not os.path.exists(qrels_file):
        print(f"\n❌ Error: Qrels file not found: {qrels_file}")
        print(f"   Please ensure the CQADupStack {subset} dataset is properly extracted.")
        sys.exit(1)
    
    # Step 1: Load all queries
    print(f"\n1. Loading all queries from {subset}/queries.jsonl...")
    all_queries = load_all_queries(queries_file)
    print(f"   Loaded {len(all_queries)} queries")
    
    # Step 2: Load relevance judgments
    print(f"\n2. Loading relevance judgments from {subset}/qrels/test.tsv...")
    ...
    opensearch_data, queries_with_judgments = create_opensearch_judgments_format(qrels, all_queries, subset)
    print(f"   Formatted {len(opensearch_data['judgmentRatings'])} queries with judgments")
    print(f"   Note: CQADupStack uses binary scale (0=not relevant, 1=relevant)")
    ...
    print("=" * 80)
    print(f"Subset: CQADupStack {subset.title()}")
