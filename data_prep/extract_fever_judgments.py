#!/usr/bin/env python3
"""
Extract FEVER relevance judgments with ultra-aggressive sanitization for OpenSearch.
...
def load_qrels(qrels_file: str) -> Dict[str, List[Dict[str, str]]]:
    ...
    Maps FEVER's binary scale to OpenSearch's scale:
    - 0 → 0.0 (not relevant)
    - 1 → 1.0 (relevant)
    ...
    # Rating mapping from FEVER binary scale to OpenSearch scale
    rating_map = {
        0: 0.0,    # Not relevant
        1: 1.0     # Relevant
    }
...
def create_opensearch_judgments_format(qrels: Dict, all_queries: Dict) -> Dict:
    ...
    opensearch_judgments = {
        "name": "FEVER Relevance Judgments",
        "description": f"FEVER (Fact Extraction and VERification) containing relevance judgments for {len(judgment_ratings)} fact-checking queries from the BEIR benchmark dataset",
...
def main():
    """Main function to extract and format FEVER judgments."""
    
    # File paths
    queries_file = "datasets/fever/queries.jsonl"
    qrels_file = "datasets/fever/qrels/test.tsv"  # Using test.tsv which has 6,666 unique queries
    output_file = "results/fever_judgments.json"
    
    print("=" * 80)
    print("FEVER Judgment Extraction with ULTRA-AGGRESSIVE Sanitization")
    print("=" * 80)
    ...
    print(f"   Note: FEVER uses binary scale (0=not relevant, 1=relevant)")
