#!/usr/bin/env python3
"""
Extract FEVER queries with ultra-aggressive sanitization for OpenSearch.
...
def load_qrels_query_ids(qrels_file: str) -> Set[str]:
    """Load unique query IDs from the qrels TSV file."""
    query_ids = set()
...
def create_opensearch_import_format(queries: Dict[str, str], qrels_query_ids: Set[str]) -> Dict:
    """Create the OpenSearch Search Relevance Workbench import format.
    Only includes queries that have ratings in the qrels file."""
    ...
    # Sort by query ID for consistent ordering, but only process queries with ratings
    for query_id in sorted(queries.keys()):
        # Skip queries that don't have ratings
    ...
    opensearch_import = {
        "name": "fever_queries",
        "description": f"FEVER (Fact Extraction and VERification) containing {len(query_set_queries)} fact-checking queries from the BEIR benchmark dataset",
        "sampling": "manual",
...
def main():
    """Main function to extract and format FEVER queries."""
    
    # File paths
    queries_file = "datasets/fever/queries.jsonl"
    qrels_file = "datasets/fever/qrels/test.tsv"  # Using test.tsv with 6,666 unique queries
    output_file = "results/fever_queries.json"
    
    print("=" * 80)
    print("FEVER Query Extraction with ULTRA-AGGRESSIVE Sanitization")
    print("=" * 80)
    
    # Check if file exists
    if not os.path.exists(queries_file):
        print(f"\n❌ Error: File not found: {queries_file}")
        print("   Please ensure the FEVER dataset is properly extracted.")
        return
    
    # Step 1: Load query IDs from qrels file
    print("\n1. Loading query IDs from qrels file...")
    qrels_query_ids = load_qrels_query_ids(qrels_file)
    print(f"   Found {len(qrels_query_ids)} unique queries with ratings")
    
    # Step 2: Load all queries
    print("\n2. Loading FEVER queries...")
    queries = load_all_queries(queries_file)
    print(f"   Total queries loaded: {len(queries)}")
    
    # Filter to only queries with ratings
    filtered_queries = {qid: text for qid, text in queries.items() if qid in qrels_query_ids}
    print(f"   Filtered to {len(filtered_queries)} queries that have ratings")
    
    # Step 3: Show sample queries
    print("\n3. Sample ORIGINAL queries (first 3):")
    sample_queries = list(filtered_queries.items())[:3]
    ...
    opensearch_data = create_opensearch_import_format(filtered_queries, qrels_query_ids)
