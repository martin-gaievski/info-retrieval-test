#!/usr/bin/env python3
"""
Extract CQADupStack queries with ultra-aggressive sanitization for OpenSearch.
...
def create_opensearch_import_format(queries: Dict[str, str], subset: str) -> Dict:
    ...
    opensearch_import = {
        "name": f"cqadupstack_{subset}_queries",
        "description": f"CQADupStack {subset_display} containing {len(query_set_queries)} Stack Exchange {subset_display} queries from the BEIR benchmark dataset",
        "sampling": "manual",
...
def main():
    """Main function to extract and format CQADupStack queries."""
    
    # Parse command-line arguments
    parser = argparse.ArgumentParser(
        description='Extract CQADupStack queries for OpenSearch import',
...
Examples:
  ...
  python3 {sys.argv[0]} -s gaming
    ...
    output_file = f"results/cqadupstack_{subset}_queries.json"
    
    print("=" * 80)
    print(f"CQADupStack {subset.title()} Query Extraction with ULTRA-AGGRESSIVE Sanitization")
    print("=" * 80)
    ...
    print(f"\n1. Loading {subset.title()} queries...")
    ...
    opensearch_data = create_opensearch_import_format(queries, subset)
